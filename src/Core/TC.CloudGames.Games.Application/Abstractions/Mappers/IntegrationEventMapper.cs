namespace TC.CloudGames.Games.Application.Abstractions.Mappers
{
    public static class IntegrationEventMapper
    {
            ...
            where TIntegrationEvent : BaseIntegrationEvent
        {
            ...
            foreach (var domainEvent in domainEvents)
            {
                ...
                yield return EventContext<TIntegrationEvent>.Create<GameAggregate>(
                    data: integrationEvent,
                    aggregateId: aggregate.Id,
                    userId: userContext.Id.ToString(),
                    isAuthenticated: userContext.IsAuthenticated,
                    correlationId: userContext.CorrelationId,
                    source: $"Games.API.{handlerName ?? "UnknownHandler"}.{integrationEvent.GetType().Name}"
                );
            }
        }
    }
